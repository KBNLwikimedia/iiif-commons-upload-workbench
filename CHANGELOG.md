...
- **Existing Commons category offered from Wikidata (OI-68 Phase A)** — the manuscript's Wikidata item (auto-found by signature) very often carries P373, the Commons category that *already exists* under a different naming convention (e.g. KW 76 E 5 → `Den Haag KB 76 E 5`, KW 129 A 10 → `Lancelotcompilatie KB 129A10`). The Categories section now shows a highlighted "★ Wikidata says this manuscript already has a Commons category … · Use this category" line whenever it differs from the current value — one click adopts it (replacing the suggestion; a manuscript gets exactly one category), preventing near-duplicate categories. Wikidata candidates with a Commons gallery also get a "Commons ↗" link.
