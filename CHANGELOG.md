...
- **Removed unused local variables, dead imports & dead functions (`js/unused-local-variable`, part of OI-86)** — cleared the code-quality findings. Variables/imports: dead imports (`deleteDraft`, `extractSequenceBasename` in `app.jsx`), an unused `useState` setter and a write-only state value, an unused React-hook alias and arrow fn in `table.jsx`, an unused computation in `thumb.jsx`, and two dead `import React` lines (`icons.jsx`, `empty-hero.jsx` — the automatic JSX runtime needs React in scope only for explicit `React.*` refs). Dead functions (never referenced anywhere — one was even labelled "dead code" in a neighbouring comment): `ListView` (`app.jsx`), and `scalarOrDash`, `SelectEditor`, `ColumnMenu` (`table.jsx`), ~195 lines total; three stale comments that named the removed functions were updated. No behaviour change (all were never rendered/called); build + tests + live render verified.
