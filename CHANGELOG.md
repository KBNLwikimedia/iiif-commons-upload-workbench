...
- **Provider profile picker (scaffolding for OI-78 #78)** — the import step now shows a "Collection" chooser with the **Koninklijke Bibliotheek** (selected) and **eCodices NL** (disabled, "Coming soon") profiles, each with its logo. Only KB is functional for now; the eCodices profile previews the multi-provider direction. Logos are bundled locally (`public/providers/`); the new `src/providers.js` holds the profile definitions (id, branding, manifest hosts) for the future per-provider derivation/license/field-map work.
