...
- **Recent-manifest list is unbounded** — the previous cap of 10 entries is lifted; the list now keeps every manifest you've loaded (per-item × and Clear all still prune it). It's small user-authored state (url + signature + title + thumb per entry) in Preferences.json.
