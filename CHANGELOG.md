...
- **Select step: filter the grid to just the collisions** — next to Select all / none / Invert, the toolbar now shows **"Duplicate names (N)"** and **"Duplicate images (N)"** toggles (only when such collisions exist) that narrow the grid to only those thumbnails; the active toggle adopts the collision colour (red / orange), a "Show all" clears it, and the counter shows a "filtered ·" prefix. Selection state is untouched by filtering.
